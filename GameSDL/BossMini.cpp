...
bool BossMini::LoadImg(std::string path, SDL_Renderer* screen)
{
...
void BossMini::Show(SDL_Renderer* des)
{
    if (think_time_ == 0)
    {
...
void BossMini::ImpMoveType(SDL_Renderer* des,const float& x)
{
    if (on_ground_ == true)
    {
        if (x_pos_ >= x)
...
void BossMini::DoPlayer(Map& g_map)
{
    if (think_time_ == 0)
    {
        x_val_ = 0;
...
void BossMini::CenterEntityOnMap(Map& g_map)
{
...
void BossMini::RemoveBullet(const int& idx)
{
...
void BossMini::InitBullet(BulletObject* p_bullet, SDL_Renderer* screen)
{
    if (p_bullet != NULL)
    {
...
void BossMini::CheckToMap(Map& g_map)
{
    int x1 = 0;
    int x2 = 0;
    int y1 = 0;
...
void BossMini::MakeBullet(SDL_Renderer* des, const int& x_limit, const int& y_limit, const float& x2, const float& y2, Map& map_data)
{
    if (x2 <= x_pos_)
    {
        LoadImg("img//threat_level.png", des);
...
SDL_Rect BossMini::GetRectFrame()
{
    SDL_Rect rect;
    rect.x = rect_.x;
    rect.y = rect_.y;
